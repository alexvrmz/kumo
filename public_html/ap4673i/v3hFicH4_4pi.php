...
if($axc0 === "v3hFh4"){
	...
	$CvH3 = $cnnx4s->query($QvH3) or die ("Falló listado de Autos" . $QvH3);
	$u5hy68 = [];
	while($LVh3C = $CvH3->fetch_assoc()){
		...
		$CvH4 = $cnnx4s->query($QvH4) or die ("Falló al cargar marcas: ".$QvH4);
		...
		$CvH5 = $cnnx4s->query($QvH5) or die ("Falló al cargar modelos: ".$QvH5);
		...
		$CvH7 = $cnnx4s->query($QvH7) or die ("Falló al cargar kilometrajes: ".$QvH7);
		$t4rgt7 = [];
		...
		$CvH8 = $cnnx4s->query($QvH8) or die ("Falló al cargar kilometrajes: ".$QvH8);
		$RcRG = [];
		$i = 0;
		$xCRN = 0;
		...
		$S001 = $cnnx4s->query($C001) or die ("Fallo al seleccionar relaciones: ".$C001);
		$DR = $S001->fetch_assoc();
		$ER = $S001->num_rows;
		if($ER != 0){
			$dduid = $DR['UEFUxTU'];
	...
	/// --- TRAMITES
		$NDF = 0;
		$C006 = "SELECT * FROM c4TD0cxxV WHERE xXUNVrSXx = $xXUNVrSXx AND d0C_xDtv = 1 AND d0C_xDED0 = 1";
		$S006 = $cnnx4s->query($C006) or die ("Fallo al consultar tipos tramites: ".$C006);
		while ($DdCT = $S006->fetch_array()) {
			...
			$S005 = $cnnx4s->query($C005) or die ("Fallo seleccion de tramites: ".$C005);
			$nT = $S005->num_rows;
	...
	/// --- SERVICIOS   
		$NSF = 0;
		$C008 = "SELECT * FROM c4TS3rxxV WHERE s3R_xDtv = 1 AND s3R_xDED0 = 1 AND xXUNVrSXx = $xXUNVrSXx";
		$S008 = $cnnx4s->query($C008) or die ("Fallo al consultar servicios: ".$C008);
		while($DDS8 = $S008->fetch_array()) {
			...
			$S003 = $cnnx4s->query($C003) or die ("Fallo al consultar documentos de servicios: ".$C003);
		...
		$S002 = $cnnx4s->query($C002) or die ("Fallo al consultar servicios: ".$C002);
	...
	/// --- SERVICIOS FIN
		...
		$CvH6 = $cnnx4s->query($QvH6) or die ("Falló al cargar Trámites: ".$QvH6);
		$t4rgt4 = [];
		$nD0V = 0;
		$doxc = [];
		...
		while ($d0c5 = $CvH6->fetch_array()) {
			...
			$S010 = $cnnx4s->query($C010) or die ("Fallo al consultar tipo trámite: ".$C010);
		...
		/*while($d0c5 = $CvH6->fetch_array()){
			...
			//if(array_key_exists($d0c5['D0c7ip0xS'], $D0c7ip0s)){
				...
				$NgYhNb6 = 'tmp/'.$_SESSION['u5hID8ir5'].'/'.$fgt;
...
elseif($axc0 === 's3RvAdd'){
	if(!empty($ags_x001) && !empty($ags_x006) && !empty($ags_x007) && !empty($ags_x004) && !empty($ags_x008)){
		...
		$S004 = $cnnx4s->query($C004) or die ("Fallo al seleccionar documentos: ".$C004);
		while($DcDD = $S004->fetch_array()){
			...
			$axc0 = 'actualizar';
			$p4r4 = 'D0cIDxS = '.$idD;
			x3J3cU74DB('D0cUM3n705', $ar, $axc0, $p4r4);
			bi74c0('4ct-s3r-'.$idD, 'Se desactivo el documento:'.$idD, '');
		}

		$C009 = "SELECT * FROM c4TS3rxxV WHERE s3R_xDID = $ags_x007";
		$S009 = $cnnx4s->query($C009) or die ("Fallo al seleccionar tipo de documento: ".$C009);
	...
	r3Diri6ir('../v75t4?axc0=v3hFh4&vh3iP='.$eCry($ags_x006));
...
elseif($axc0 === 'd0CvAdd'){
	...
	r3Diri6ir('../v75t4?axc0=v3hFh4&vh3iP='.$eCry($agd_x006));
...
elseif($axc0 === '1054800'){
    ...
    $filePath = '../tmp/'.$_SESSION['u5hID8ir5'].'/'.$fileName;
...
elseif($axc0 === 'hY776'){
	...
	r3Diri6ir('../v75t4?axc0=v3hFh4&vh3iP='.$eCry($pp_xx0101));
}
elseif($axc0 === 'hY778'){
	...
	$S014 = $cnnx4s->query($C014) or die ("Fallo al consultar kilometraje: ".$C014);
	...
	$_SESSION['FRM_hY778']['frm_rgcmb001'] = Cl34N($frm_rgcmb001);
	$_SESSION['FRM_hY778']['frm_rgcmb002'] = $frm_rgcmb002;
	$_SESSION['FRM_hY778']['frm_rgcmb003'] = $frm_rgcmb003;
	$_SESSION['FRM_hY778']['frm_rgcmb004'] = strtoupper(Cl34N($frm_rgcmb004));
	$_SESSION['FRM_hY778']['frm_rgcmb006'] = Cl34N($frm_rgcmb006);
	$_SESSION['FRM_hY778']['frm_rgcmb009'] = Cl34N($frm_rgcmb006);
	...
	r3Diri6ir('../v75t4?axc0=xkmtr4G&vh3UjI6Did='.$eCry($frm_rgcmb007));
	/*echo '<pre>';
	print_r($sQl_d474_4rr4y);
	echo '</pre>';*/
}