...
if($axc0 === "pR0VxD"){
	...
	if(!empty($px019)){
		...
		foreach ($px019 as $key) {
			$C004 = "SELECT s3R_xDID, s3R_xDd35, s3R_xDtv FROM c4TS3rxxV WHERE xXUNVrSXx = $xXUNVrSXx";
			if($key != 'todos' && $key != 'ninguno'){
				$C004 .= "WHERE s3R_xDtv = $key AND ";
				$C004 .= "xXUNVrSXx = $xXUNVrSXx";
			}
			elseif($key == 'ninguno'){

			}
			$S004 = $cnnx4s->query($C004) or die ("Fallo al seleccionar servicios: ".$C004);
			...
			$C006 = "SELECT d0C_xDID, d0C_xDd35, d0C_xDtv FROM c4TD0cxxV WHERE xXUNVrSXx = $xXUNVrSXx";
			if($key != 'todos' && $key != 'ninguno'){
				$C006 .= "WHERE d0C_xDtv = $key AND ";
				$C006 .= "xXUNVrSXx = $xXUNVrSXx";
			}
			elseif($key == 'ninguno'){

			}
			$S006 = $cnnx4s->query($C006) or die ("Fallo al seleccionar Tramites: ".$C006);
			...
			$S005 = $cnnx4s->query($C005) or die ("Fallo al seleccionar marcas: ".$C005);
...
elseif ($axc0 == 'aDdPr0') {
	...
	$pp_xx012 = strtoupper(Cl34N($pp_xx012));
	...
	if($_SESSION['m3n3Rr0R'] != 'si'){
		...
		$np = x3J3cU74DB('pR0xV3D', $sQl_d474_4rr4y, $axc0, $p4r4m37r05);
		include('../escen78/iDi0m45/'.$_SESSION['iDi0m4'].'/i_8i74c0r4.php');
		...
		if(!empty($pp_xx017) && !in_array("todos", $pp_xx017) && !in_array("ninguno", $pp_xx017)){
			foreach ($pp_xx017 as $key) {
				...
				x3J3cU74DB('r3l3PSr', $sQl_d474_4rr4y, $axc0, $p4r4m37r05);
			}
		}
		...
		if(!empty($pp_xx020) && !in_array("todos", $pp_xx020) && !in_array("ninguno", $pp_xx020)){
			foreach ($pp_xx020 as $key) {
				...
				x3J3cU74DB('r3l3PTr', $sQl_d474_4rr4y, $axc0, $p4r4m37r05);
			}
		}
		...
		if(!empty($pp_xx018) && !in_array("todos", $pp_xx018) && !in_array("ninguna", $pp_xx018)){
			foreach ($pp_xx018 as $key) {
				...
				x3J3cU74DB('r3l3PM', $sQl_d474_4rr4y, $axc0, $p4r4m37r05);
			}
		}
		...
		if(!empty($pp_xx019 && !in_array("todos", $pp_xx019) && !in_array("ninguno", $pp_xx019))){
				...
				foreach ($pp_xx019 as $key) {
					...
					x3J3cU74DB('r3l3PTV', $sQl_d474_4rr4y, $axc0, $p4r4m37r05);
				}
...
elseif($axc0 == 'pr0VxVW' || $axc0 == 'pr0VxDT'){
	if(!empty($pXRoI)){
		...
		$S006 = $cnnx4s->query($C006) or die ("Fallo al seleccionar relaciones tipo de vehiculos: ".$C006);
		$LDTP = [];
		...
		$S007 = $cnnx4s->query($C007) or die ("Fallo al seleccionar relaciones de servicios: ".$C007);
		$LDTS = [];
		while ($DDTS = $S007->fetch_array()) {
			...
			$S008 = $cnnx4s->query($C008) or die ("Fallo al seleccionar servicio: ".$C008);
		...
		$S010 = $cnnx4s->query($C010) or die ("Fallo al seleccionar relaciones de trámites: ".$C010);
		$LDTT = [];
		while ($DDTT = $S010->fetch_array()) {
			...
			$S011 = $cnnx4s->query($C011) or die ("Fallo al seleccionar tramites: ".$C011);
		...
		$S009 = $cnnx4s->query($C009) or die ("Fallo al seleccionar relaciones de marcas: ".$C009);
		$LDMk = [];
		...
		$C012 = "SELECT * FROM c4TD0cxxV WHERE xXUNVrSXx = $xXUNVrSXx AND d0C_xDED0 = 1";
		$S012 = $cnnx4s->query($C012) or die ("Fallo error al seleccionar tramites: ".$C012);
		$ALDT = [];
		...
		$C013 = "SELECT * FROM c4TS3rxxV WHERE xXUNVrSXx = $xXUNVrSXx AND s3R_xDED0 = 1";
		$S013 = $cnnx4s->query($C013) or die ("Fallo error al seleccionar servicios: ".$C013);
		$ALDS = [];
		...
		$S014 = $cnnx4s->query($C014) or die ("Fallo error al seleccionar marcas: ".$C014);
		$ALDM = [];
...
elseif($axc0 == 'sV3DT'){
	if($pXRoI == ''){
		...
		$frm_003 = strtoupper(Cl34N($frm_003));
		$frm_004 = Cl34N($frm_004);
		$frm_005 = Cl34N($frm_006);
		...
		if($_SESSION['m3n3Rr0R_num'] === 0){
			...
			if($_SESSION['m3n3Rr0R_num'] === 0){
				...
				x3J3cU74DB('pR0xV3D', $aRrr4y, $axc0, $p4r4m37r05);
...
elseif($axc0 == 'Ac7r3L'){
	...
	x3J3cU74DB('pR0xV3D', $sQl_d474_4rr4y, $axc0, $p4r4m37r05);
	...
	$S016 = $cnnx4s->query($C016) or die ("Fallo al seleccionar Relacion servicios: ".$C016);
	...
	$S017 = $cnnx4s->query($C017) or die ("Fallo al seleccionar Relacion tramites: ".$C017);
	...
	$S018 = $cnnx4s->query($C018) or die ("Fallo al seleccionar Relacion tramites: ".$C018);
	...
	r3Diri6ir('../v75t4?axc0=pr0VxVW&pXRoI='.$eCry($pp_xx015));
