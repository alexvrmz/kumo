...
if($axc0 === "v3hiXs"){
	...
	elseif($flo0 != '' && $mdr == 1){
		...
		$QvH3 = "SELECT * FROM v3hiXu7l0s WHERE xV3hiUIDXu7l0s = '".$u5u4i0."' ";
		if($flo0 != ''){
			$fl0o0 = $dCry($flo0);
	...
	while($LVh3C = $CvH3->fetch_assoc()){
		...
		if($eMp3 == 0){
			...
			$CR67004 = $cnnx4s->query($QR67004) or die ("Falló listado de empresas" . $QR67004);
		...
		$CR67005 = $cnnx4s->query($QR6705) or die ("Falló listado de docs" . $QR6705);
		$NDos = 0;
	...
	if($_SESSION['v3_c016'] != 'nada' && !empty($_SESSION['v3_c016'])){
		...
		$C002 = $cnnx4s->query($Q002) or die ("Falló listado de relaciones 1: " . $Q002);
		$fl0 = [];
		while($fl = $C002->fetch_assoc()){
			$fl7iD = $fl['r3FFloO00'];
			$QR6704 = "SELECT xID7l0, xN4m37l0 FROM x7l07 WHERE xID7l0 = ".$fl7iD." ";
			$CR67004 = $cnnx4s->query($QR6704) or die ("Falló listado de flotas" . $QR6704);
	...
	$Q001 = "SELECT eMIDEm, eMRZEm, eMNICEm FROM eMpR3sA5 WHERE eMiDuS3Em = '".$u5u4i0."' ";
	...
	while($eMy6 = $C001->fetch_assoc()){
		...
		$C003 = $cnnx4s->query($Q003) or die ("Falló listado de relaciones2" . $Q003);
		$nuR3 = $C003->num_rows;
...
elseif($axc0 === "v3hFh4_sd04"){
    ...
    $c5u5xXd = $cnnx4s->query($q5u5xXd) or die ("Falló al obtener datos de Auto " . $q5u5xXd);
    $d4705_u5u = [];
...
elseif ($axc0 == 'sDrft5') {
	...
	$C006 = "SELECT xV3hivINXu7l0s, xV3hiPl4c45Xu7l0s, xV3him070rXu7l0s, xV3hi3C0Xu7l0s FROM v3hiXu7l0s WHERE xXUNVrSXx = $xXUNVrSXx"; // WHERE xV3hivINXu7l0s = '".."' OR xV3hiPl4c45Xu7l0s = '".."' ";
		$S006 = $cnnx4s->query($C006) or die ("Fallo al consultar VIN y Matricula");
		while ($DV = $S006->fetch_assoc()) {
			$v = dCry2($DV['xV3hivINXu7l0s']);
			$p = dCry2($DV['xV3hiPl4c45Xu7l0s']);
	...
	if($_SESSION['m3n3Rr0R'] != 'si'){
		if(	!empty($v3_c001) && !empty($v3_c002) && !empty($v3_c003) && !empty(is_numeric($v3_c016)) && !empty(is_numeric($v3_c015)) && 
				...
				!empty(is_numeric($v3_c013)) && !empty(is_numeric($v3_c014)) &&	!empty(is_numeric($u5u4i0)) ) {
				...
				$fdcH = date('Y-m-d H:i:s');
					$sQl_d474_4rr4y = [
						...
						'xV3hiUIDXu7l0s' => $_SESSION['u5hID8ir5'],
...
elseif($axc0 == 'xkmtr4G'){
	if(!empty($vh3UjI6Did)){
		...
		$S013 = $cnnx4s->query($C013) or die ("Fallo al consultar Vehiculo: ".$C013);
		...
		if(!empty($_SESSION['fil_FHF']) && !empty($_SESSION['fil_FHI'])){
			...
			$CR67003 = $cnnx4s->query($QR67003) or die ("Falló listado de kilometrajes" . $QR67003);
		}
		else {
		...
		$CR67004 = $cnnx4s->query($QR67004) or die ("Falló listado de kilometrajes" . $QR67004);
...
elseif($axc0 === 'd3svA'){
	...
	if($aIDdx != '' && empty($uxu) && empty($r3l)){
		...
		$S008 = $cnnx4s->query($C008) or die ("Fallo al consultar relacion de auto: ".$C008);
