...
if($axc0 === "bin4kuru"){
	...
	$S001 = $cnnx4s->query($C001) or die ("Fallo al consultar bitacora: ".$C001);
	$ldi = [];
	while ($DdB = $S001->fetch_assoc()) {
		...
		if(!empty($kuUSIDx)){
			$C002 = "SELECT u5hUS8ir5 FROM u5u405 WHERE u5hID8ir5 = $kuUSIDx";
			$S002 = $cnnx4s->query($C002) or die ("fallo al seleccionar usuario: ".$C002);
			$DDU = $S002->fetch_assoc();
			$EdUS =  $DDU['u5hUS8ir5'];
		}

		if(!empty($kuUUIDx)){
			$C003 = "SELECT u5hUS8ir5 FROM u5u405 WHERE u5hID8ir5 = $kuUUIDx";
			$S003 = $cnnx4s->query($C003) or die ("fallo al seleccionar usuario: ".$C003);
		...
		if(!empty($kuEMDx)){
			...
			$S004 = $cnnx4s->query($C004) or die ("Fallo al seleccionar empresa: ".$C004);
		...
		if(!empty($kuFLIDx)){
			...
			$S005 = $cnnx4s->query($C005) or die ("Fallo al seleccionar flota: ".$C005);
		...
		if(!empty($kuDIDx)){
			...
			$S006 = $cnnx4s->query($C006) or die ("Fallo al seleccionar documento: ".$C006);
		...
		if(!empty($kuVEIDx)){
			...
			$S007 = $cnnx4s->query($C007) or die ("Fallo al seleccionar Automovil: ".$C007);
