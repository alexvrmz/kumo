...
if($axc0 === "5u550l"){
	...
	$Cm0d78Os = $cnnx4s->query($Qm0d78Os) or die ("Falló listado de modulos" . $Qm0d78Os);
	$L74m0d78Os = [];
	...
	$CseP3rMi505 = $cnnx4s->query($QseP3rMi505) or die ("Falló listado de permisos" . $QseP3rMi505);
	$LseP3r505 = [];
	while($LsxP3r705 = $CseP3rMi505->fetch_assoc()){
		...
		$Cm0d78O2 = $cnnx4s->query($Qm0d78O2) or die ("Falló listado de modulos" . $Qm0d78O2);
...
elseif ($axc0 == 'xu5sdS7l') { /// --- Accion cargar datos de permiso
	$p3ry6I = $dCry($p3ry6I);
	if(!empty($p3ry6I)){
    // --- Consulta para datos de usuario
    $q5u5xXd = "SELECT * FROM p3rM5sx_e WHERE p3ry6IDXx = '".$p3ry6I."' ";
    $c5u5xXd = $cnnx4s->query($q5u5xXd) or die ("Falló al obtener datos de permiso " . $q5u5xXd);
    $d4705_u5u = [];
...
elseif ($axc0 == 'p3rf5Gg') { /// --- Accion actualizar
...
elseif ($axc0 === '5uXds4H') {
    ...
    $c5u5xXd = $cnnx4s->query($q5u5xXd) or die ("Falló al obtener permisos de usuario " . $q5u5xXd);
    $d4705_u5u = [];
    while($dU5u4o = $c5u5xXd->fetch_assoc()){
    	...
    	$c5u5xdsXd = $cnnx4s->query($q5u5xXer4) or die ("Falló al obtener permisos de usuario " . $q5u5xXer4);
...
elseif ($axc0 === 'sD4xG5d5') { /// --- carga permisos por modulo
    ...
    $c5u5xXd = $cnnx4s->query($q5u5xXd) or die ("Falló al obtener permisos " . $q5u5xXd);
    $d4705_u5u = [];
    while($dU5u4o = $c5u5xXd->fetch_assoc()){
...
elseif ($axc0 == 'psDef4Gg') { /// --- Accion actualizar estado permiso
	$h7gr =  $dCry($h7gr);
	$txgt5 = $dCry($txgt5);
	$u5u4oxX = $dCry($u5u4oxX);
