...
if($axc0 === "eMpr35As" ){
	...
	if($px010 != ''){
		...
		$CQ3s747 = $cnnx4s->query($SQ3s747) or die ("Falló listado de colonias " . $SQ3s747);
		$kol0 = [];
	...
	if($flo0 != ''){
		...
		//	$flo0 = $dCry($flo0);
			...
			$C005 = $cnnx4s->query($Q005) or die ("Falló listado de empresas1" . $Q005);
		...
		if($_SESSION['v3_c016'] != ''){
			...
			$C002 = $cnnx4s->query($Q002) or die ("Falló listado de relaciones1" . $Q002);
			$fl0 = [];
		...
		$Q001 = "SELECT eMIDEm, eMRZEm, eMNICEm FROM eMpR3sA5 WHERE xXUNVrSXx = '".$xXUNVrSXx."' ";
...
elseif ($axc0 == 'eMprxXx21') {
	...
	elseif(!V4lRQ($pp_xx012)){
		...
		$CQ3s725 = $cnnx4s->query($SQ3s725) or die ("Falló listado de empresas" . $SQ3s725);
...
elseif($axc0 == 'emPr3Edi7'){
	if(!empty($eMpr3)){
		...
		$S002 = $cnnx4s->query($C002) or die ("Fallo al consultar empresa: ".$C002);
		$DP = $S002->fetch_assoc();
		$xPxIDH = $DP['eMIDEm'];
...
elseif($axc0 == 'sV3EMp'){
	if($eMpr3 == ''){
		...
		if($_SESSION['m3n3Rr0R_num'] === 0){
			...
			$S003 = $cnnx4s->query($C003) or die ("Fallo al consultar empresas: ".$C003);
