...
function v4lID44x50($fhn4, $u53) {
  ...
  $Csd7Hd5td = $cnnx4s->query($Qsd7Hd5td) or die ("Falló listado de permisos" . $Qsd7Hd5td);
  $p = $Csd7Hd5td->fetch_array();

  $Qjdf87 = "SELECT * FROM p3rXo70R64D0s WHERE p3rXou5Rs = '".$u53."' AND p3rXoNmRs = '".$p['p3ry6IDXx']."' ";
  $Cjdf87 = $cnnx4s->query($Qjdf87) or die ("Falló listado de usuarios" . $Qjdf87);
    ...
    if($action == 'insertar'){
     ...
     // while(list(, $value) = each($data)){
      ...
      $query = 'DELETE FROM ' . $table . ' WHERE ' . $parameters;
        ...
        //    echo $query;
    $cnnx4s -> query($query) or die($mysqli_error($cnnx4s));
    $result = $cnnx4s->insert_id;
    return $result;
...
function su8iM6($Fxc47374, $Fxd3scR74) {
  ...
  /// --- URL ENC
      ...
      if($_SERVER['SERVER_NAME'] == 'mieke.mx' || $_SERVER['SERVER_NAME'] == 'nopixcua.kumo.click'){
        $puk3y = file_get_contents('/home/nopixcua/cdr-e/u5u4ri05/5y5/x0101_pu.pem');
        $prk3y = file_get_contents('/home/nopixcua/cdr-e/u5u4ri05/5y5/x0101_pv.pem');
        $grdV = '/home/nopixcua/cdr-e/u5u4ri05/';
      }
      elseif($_SERVER['SERVER_NAME'] == 'localhost' || $_SERVER['SERVER_NAME'] == '192.168.1.80'){
        $puk3y = file_get_contents('/home/alejandro/sitios/cdr-e/u5u4ri05/5y5/x0101_pu.pem');
        $prk3y = file_get_contents('/home/alejandro/sitios/cdr-e/u5u4ri05/5y5/x0101_pv.pem');
        $grdV = '/home/alejandro/sitios/cdr-e/u5u4ri05/'; //Usuario: nopixcua Clave: Wj0AN6Q79vueuaI 
      }
...
function ueCry($txtt, $u5u5){
    global  $cnnx4s; global $grdV;
    $SQjhg = "SELECT * FROM L4v3s WHERE L4v3s_id = '".$u5u5."' ORDER BY L4v3s_id DESC LIMIT 1";
		$CQjhg = $cnnx4s->query($SQjhg) or die ("Fallo al ontener llaves");
...
function gLl4v3($ifg, $pvk, $puk){
    ...
    $n0m8r3 = '../u5u4ri05/'.$ifg.'/pvk-'.$txm.'-'.$ifg.'.pem';
    $n0m8r2 = '../u5u4ri05/'.$ifg.'/puk-'.$txm.'-'.$ifg.'.pem';
    $c4rp374 = '../u5u4ri05/'.$ifg;
...
function c0nUs($flo0 = 0, $ePr3 = 0, $xux = 0){
    global $cnnx4s;
		if($flo0 != 0 && $ePr3 == 0 && $xux == 0){
			...
			$Q001 = $cnnx4s->query($S001) or die ("Falló Obtener datos de Flota");
			$DL = $Q001->fetch_assoc();
			$iI = $DL['x3MPID7l0'];
			$STE = 'UEFFxTU';
			$OUJ = $flo0;
		...
		elseif($ePr3 != '' || $ePr3 != 0){
			...
			$Q001 = $cnnx4s->query($S001) or die ("Falló Obtener datos de Empresa");
			$DL = $Q001->fetch_assoc();
			$iI = $DL['eMIDEm'];
			$STE = 'UEFExTU';
			$OUJ = $ePr3;
    ...
    elseif($xux !== 0){
      $C001 = "SELECT * FROM u5u405 WHERE u5hID8ir5 = $xux";
      $S001 = $cnnx4s->query($C001) or die ("Falló al consultar datos de usuario");
      $DU = $S001->fetch_assoc();
      $u5hy68 = [];
      $xVnUDnVx = $DU['u5hID8ir5'];
      $xVUSnVx = $DU['u5hUS8ir5'];
      $xVN1nVx = dCry2($DU['u5hN18ir5']);
      $xVN2nVx = dCry2($DU['u5hN28ir5']);
      $xVA1nVx = dCry2($DU['u5hA18ir5']);
      $xVA2nVx = dCry2($DU['u5hA28ir5']);
    ...
    if(($flo0 != 0 || $ePr3 != 0) && $xux === 0){
      ...
      $S002 = $cnnx4s->query($Q002) or die ("Fallo al obtener relaciones de usuario: <b>".$Q002.'</b>'); 
      $u5hy68 = [];
      while ($DR = $S002->fetch_assoc()) {
        $U = $DR['UEFUxTU'];
        $Q003 = "SELECT * FROM u5u405 WHERE u5hID8ir5 = $U ORDER BY u5hID8ir5 ASC";
        $S003 = $cnnx4s->query($Q003) or die ("Falló listado de usuarios: " . $Q003);
        $lista_u5u405 = $S003->fetch_assoc();
        if($lista_u5u405['u5hID8ir5'] != ''){ $ID8ir5 = /*dCry2(*/$lista_u5u405['u5hID8ir5']/*)*/; }
        ...
        if($lista_u5u405['u5hA18ir5'] != ''){ $A18ir5 = dCry2($lista_u5u405['u5hA18ir5']); }
        if($lista_u5u405['u5hA28ir5'] != ''){ $A28ir5 = dCry2($lista_u5u405['u5hA28ir5']); }
        if($lista_u5u405['u5hN18ir5'] != ''){ $A28ir5 = dCry2($lista_u5u405['u5hN18ir5']); }
        if($lista_u5u405['u5hN28ir5'] != ''){ $N28ir5 = dCry2($lista_u5u405['u5hN28ir5']); }
...
function c0nNumUs($flo0 = 0, $ePr3 = 0){
  ...
  if($flo0 != 0){
    ...
    $Q001 = $cnnx4s->query($S001) or die ("Falló obtener numero de usuarios");
  }
  elseif($ePr3 != 0){
...
function c0nNumFl($ePr3 = 0, $u5U = 0){
  ...
  $Q001 = $cnnx4s->query($S001) or die ("Falló obtener numero de flotas");
...
function c0nM4rk(){
  ...
  $CQm4R = $cnnx4s->query($SQm4R) or die ("Falló al cargar marcas: ".$SQm4R);
...
function c0n5ub($MK){
  ...
  $CQm4R = $cnnx4s->query($SQm4R) or die ("Falló al cargar modelos: ".$SQm4R);
...
function c0nEmP($flo0 = 0, $mdr = 0, $eid = 0){
  ...
  if(($flo0 != 0 || $flo0 != '') && $mdr == 2){
    ...
    $CQ3s702 = $cnnx4s->query($SQ3s702) or die ("Falló listado de relaciones" . $SQ3s730);

    $SQ3s730 = "SELECT xN4m37l0 FROM x7l07 WHERE xID7l0 = '".$flo0."' ";
    $CQ3s730 = $cnnx4s->query($SQ3s730) or die ("Falló listado de flotas" . $SQ3s730);
  ...
  elseif($eid != 0 && $flo0 == 0 && $mdr == 0){
    ...
    $CQ3s702 = $cnnx4s->query($SQ3s702) or die ("Falló listado de empresas" . $SQ3s702);
  }
  else{
    $SQ3s702 = "SELECT eMIDEm, eMRZEm, eMNICEm, eMRFCEm FROM eMpR3sA5 WHERE eMiDuS3Em = ".$_SESSION['u5hID8ir5']." ";
    $CQ3s702 = $cnnx4s->query($SQ3s702) or die ("Falló listado de empresas" . $SQ3s702);
  }
  ...
  while($LeMs4 = $CQ3s702->fetch_assoc()){
    ...
    if($flo0 != '' && $mdr == 2){
      ...
      $CQ3s755 = $cnnx4s->query($SQ3s755) or die ("Falló listado de empresas" . $SQ3s755);
      $dEM = $CQ3s755->fetch_assoc();
      $SQ3s725 = "SELECT * FROM r3l3mpfl00 WHERE r3F3mP00 = ".$dEM['eMIDEm']." ";
      $CQ3s725 = $cnnx4s->query($SQ3s725) or die ("Falló listado de relaciones" . $SQ3s725);
      $SQ3s735 = "SELECT xV3hi3MPrXu7l0s FROM v3hiXu7l0s WHERE xV3hi3MPrXu7l0s = ".$dEM['eMIDEm']." ";
      $CQ3s735 = $cnnx4s->query($SQ3s735) or die ("Falló listado de vehiculos" . $SQ3s735);
      $nv = $CQ3s735->num_rows;
      $arr4y = array(
        'XrIDEx' => $dEM['eMIDEm'],
        'XrRZEx' => $dEM['eMRZEm'], 
      ...
      $CQ3s755 = $cnnx4s->query($SQ3s755) or die ("Falló listado de flotas" . $SQ3s755);
...
function c0nNuEmP($flo0, $mdr){
  ...
  if($flo0 != '' && $mdr == 2){
    //$flo0 = $dCry($flo0);
    $SQ3s702 = "SELECT * FROM r3l3mpfl00 WHERE r3FFloO00 = '".$flo0."' ";
    $CQ3s702 = $cnnx4s->query($SQ3s702) or die ("Falló numero de empresas" . $SQ3s730);
    $r = 1;
  }
  else{
    $SQ3s702 = "SELECT eMIDEm FROM eMpR3sA5 WHERE eMiDuS3Em = ".$_SESSION['u5hID8ir5']." ";
    $CQ3s702 = $cnnx4s->query($SQ3s702) or die ("Falló numero de empresas" . $SQ3s702);
    $r = 2;
  }
  $ne = $CQ3s702->num_rows;
  return $ne;
...
function c0nEst4(){
  ...
  $SQ3s742 = "SELECT xXid3s742x, xXN00Ms742x FROM 3s7A2";
	$CQ3s742 = $cnnx4s->query($SQ3s742) or die ("Falló listado de estados" . $SQ3s742);
	$l3st4 = [];
...
function C0nMuniD($MD){
  ...
  $SQ3s745 = "SELECT zSMNcPi0sidX, zSMNcPi0sN0omX FROM MNcPi0s WHERE zSMNcPi0sidX = '".$MD."' ";
	$CQ3s745 = $cnnx4s->query($SQ3s745) or die ("Falló municipios " . $SQ3s745);
...
function C0nMun($px010){
  ...
  $SQ3s745 = "SELECT zSMNcPi0sidX, zSMNcPi0sN0omX FROM MNcPi0s WHERE zSMNcPi0sX3DOIDIx = '".$px010."' ";
	$CQ3s745 = $cnnx4s->query($SQ3s745) or die ("Falló listado de municipios " . $SQ3s745);
	$lPi0s = [];
...
function c0nNumAu($flo0, $ePr3){
  ...
  if($flo0 != 0){
    ...
    $Q001 = $cnnx4s->query($S001) or die ("Falló obtener numero de autos");
  }
  elseif($ePr3 != 0){
...
function C0nFl0($u5u = 0, $eMp = 0){
  ...
  if($u5u != 0){
    ...
    $S001 = $cnnx4s->query($C001) or die ("Fallo al seleccionar flotas");
    $fl = [];
  ...
  elseif($eMp != 0){
    ...
    $S002 = $cnnx4s->query($C002) or die ("Fallo al seleccionar flotas");
    while ($DR = $S002->fetch_assoc()) {
      $xDfir = $DR['r3FFloO00'];
...
function x_sisM($m){
  ...
  $S001 = $cnnx4s->query($C001) or die ("Fallo al consultar email: ".$C001);
  $x = $S001->num_rows;
