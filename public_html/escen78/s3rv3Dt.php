<?php
include('iDi0m45/'.$iDi['u5h8ir5_iDi'].'/i_s3rv3Dt.php');
    ...
    <div class="card <?= $brr4 ?> card-outline col-md-9">
      ...
      <div class="card-body">
        <div class="card-body p-0">         
          <a type="button" class="btn bg-gradient-success btn-app" style="padding-top: 6px;" href="v75t4?axc0=s3rv"><i class="fa-duotone fa-arrow-left"></i> <br><?= btn_46r364rU5u4ri0 ?></a>
        </div>
      </div>
    </div>
      <div class="card <?= $brr4 ?> card-outline col-md-9">
        <div class="card-header">
          <h3 class="profile-username text-center">Editando Servicio | <?= $xSrD3sp0 ?></h3>  
        </div>
        ...
        <form method="POST" action="ap4673i/s3rv_4pi?axc0=sV3S3rv">
          <div class="card-body">
            ...
            <div class="form-group">
              <label for="frm_003">Alerta Otro (Km)</label>
              <?php if(!empty($_SESSION['tXFrm']['frm_003'])){ $vrffrm_003 = 'is-invalid'; } else { $vrffrm_003 = 'is-valid'; }?>
              <?php if($frm_003 != ''){ $vt_frm_003 = $dCry($frm_003); } else { $vt_frm_003 = $xSrOTp0; } ?>
              <input type="text" class="form-control <?= $vrffrm_003 ?>" id="frm_003" name="frm_003" value="<?= $vt_frm_003 ?>">
            </div>
